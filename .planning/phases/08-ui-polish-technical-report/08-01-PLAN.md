...
must_haves:
  truths:
    ...
    - "At least 4 screenshots (landing, lobby, game, postgame) exist in docs/screenshots/ and are referenced by relatorio.md"
  artifacts:
    ...
    - path: "docs/screenshots/landing.png"
      provides: "Screenshot of Landing page"
    - path: "docs/screenshots/lobby.png"
      provides: "Screenshot of Lobby screen"
    - path: "docs/screenshots/game.png"
      provides: "Screenshot of GameScreen during HINT_PHASE"
    - path: "docs/screenshots/postgame.png"
      provides: "Screenshot of PostGame screen"
  key_links:
    - from: "docs/Makefile"
    ...
    - from: "docs/relatorio.md"
      to: "docs/screenshots/*.png"
      via: "Markdown image references"
      pattern: "screenshots/landing\\.png"
---

<objective>
Set up the technical report document and build pipeline.

Purpose: Create the complete docs/ scaffold — Makefile, Mermaid diagram sources, full relatorio.md content, and application screenshots — so the academic report can be compiled to PDF by running a single `make -C docs pdf` command. This is the Wave 0 deliverable for all REPORT-XX requirements.

Output: docs/relatorio.md (full content with real screenshot references), docs/Makefile, docs/puppeteer-config.json, three .mmd diagram sources, docs/package.json (for @mermaid-js/mermaid-cli dependency), docs/screenshots/ directory with at least 4 PNGs.
</objective>
...
Screenshots final paths (for relatorio.md image references):
- docs/screenshots/landing.png
- docs/screenshots/lobby.png
- docs/screenshots/game.png
- docs/screenshots/postgame.png
</interfaces>
</context>
...
Section 3 (REPORT-03): `# 3. Demonstração da Aplicação` — Include a paragraph describing each screen (Landing, Lobby, GameScreen, PostGame). Include the following image references pointing to the screenshots captured in Task 3:
...
Include 2 code excerpts with triple-backtick Python fences: (a) the BridgeCallbackReceiver pattern from CLAUDE.md §Key Pyro5 Patterns (register_callback + threading.local proxy), (b) the @oneway broadcast_test method pattern.
...
The report must NOT contain any credentials, passwords, or real secrets — use example values only (D from security context). Total length target: dense, 5-8 pages when compiled.
  ...
  <verify>
    <automated>test -f docs/relatorio.md && grep -c "# 1\." docs/relatorio.md | grep -v "^0$" && grep -q "# 2\. Arquitetura" docs/relatorio.md && grep -q "# 3\. Demonstra" docs/relatorio.md && grep -q "# 4\. Instala" docs/relatorio.md && grep -q "diagrams/arquitetura.png" docs/relatorio.md && grep -q "diagrams/seq-callback.png" docs/relatorio.md && grep -q "diagrams/seq-game-event.png" docs/relatorio.md && grep -q "screenshots/landing.png" docs/relatorio.md && grep -q "screenshots/lobby.png" docs/relatorio.md && grep -q "screenshots/game.png" docs/relatorio.md && grep -q "screenshots/postgame.png" docs/relatorio.md</automated>
  </verify>
  <acceptance_criteria>
    ...
    - Section 3 contains all 4 screenshot image references: screenshots/landing.png, screenshots/lobby.png, screenshots/game.png, screenshots/postgame.png
  ...
  <done>docs/relatorio.md written with all 4 required sections; all 4 screenshot image paths reference docs/screenshots/ files that will be created in Task 3.</done>
</task>

<task type="checkpoint:human-action">
  <name>Task 3: Capture application screenshots for the technical report (REPORT-03)</name>
  <files>
    docs/screenshots/landing.png
    docs/screenshots/lobby.png
    docs/screenshots/game.png
  ...
  <what-built>
    docs/relatorio.md (Task 2) references 4 screenshot files in docs/screenshots/. These files must exist and be real PNG screenshots of the running application so relatorio.pdf does not contain broken image references.
  </what-built>
  <how-to-verify>
    1. Ensure all 3 backend processes are running (see CLAUDE.md startup):
    ...
    3. Create the screenshots directory: `mkdir -p docs/screenshots`
    4. Take the following 4 screenshots and save them to the listed paths (use your system screenshot tool or browser DevTools → Capture screenshot):
       - `docs/screenshots/landing.png` — The Landing page (http://localhost:5173 before any action)
       - `docs/screenshots/lobby.png` — The Lobby screen (after creating or joining a game, with at least 1 player listed)
       - `docs/screenshots/game.png` — The GameScreen during HINT_PHASE (after starting the game, during the hint submission phase)
       - `docs/screenshots/postgame.png` — The PostGame screen (after a complete game round finishes)
    5. Verify all 4 files exist and are non-zero:
       `ls -lh docs/screenshots/landing.png docs/screenshots/lobby.png docs/screenshots/game.png docs/screenshots/postgame.png`
    6. Run the full report build to confirm no broken image errors:
       `make -C docs pdf && test -f docs/relatorio.pdf`
  </how-to-verify>
  <resume-signal>Type "screenshots done" once all 4 PNG files are saved to docs/screenshots/ and `make -C docs pdf` exits 0.</resume-signal>
</task>
...
- `test -d docs/screenshots && ls docs/screenshots/*.png | wc -l | grep -q '[4-9]'` — at least 4 screenshots exist
...
- docs/screenshots/ with 4 PNGs (landing, lobby, game, postgame)
- `make -C docs pdf` exits 0 and produces docs/relatorio.pdf
- Report content: RPC comparison table, 3-process architecture explanation, ≥2 sequence diagram references, 4 real screenshot references, code excerpts, installation instructions
</success_criteria>
